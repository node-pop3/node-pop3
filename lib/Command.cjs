 ...
 */
function parseCapabilities(str) {
  /** @type {Record<string, string[]>} */
  const capabilities = {};
  for (const parts of listify(str)) {
    const [name, ...values] = parts;
    if (!name) {
      continue;
    }
    capabilities[name.toUpperCase()] = values;
  }
  return capabilities;
}
...
class Pop3Command extends Pop3Connection {
  ...
  async CAPA() {
    await this._connect();
    const [, stream] = await super.command('CAPA');
    const str = await stream2String(/** @type {import('stream').Stream} */stream);
    this._capabilities = parseCapabilities(str);
  ...
  async supports(capability) {
    const normalizedCapability = capability.trim().toUpperCase();
    ...
    return Boolean(this._capabilities && Object.hasOwn(this._capabilities, normalizedCapability));
  }
  ...
  async LAST() {
    ...
    try {
      ...
      if (typeof err === 'object' && err !== null && 'message' in err && typeof err.message === 'string' && /(?:last command not enabled|unknown command)/ui.test(err.message)) {
        return err.message;
      }
      throw err;
    }
