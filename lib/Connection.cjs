...
import { CRLF, CRLF_BUFFER, TERMINATOR_BUFFER, TERMINATOR_BUFFER_ARRAY, MULTI_LINE_COMMAND_NAME, MAYBE_MULTI_LINE_COMMAND_NAME } from "./constant.cjs";
...
class Pop3Connection extends EventEmitter {
  ...
  */
   ...
   */
  _hasActiveSocket() {
    return Boolean(this._socket && !this._socket.destroyed && this._socket.writable && !this._socket.writableEnded);
  }
  ...
  connect() {
    ...
    resolve, reject) => {
      ...
      buffer => {
        ...
        if (this._pendingResponseBuffer.length) {
          const len = this._pendingResponseBuffer.length + buffer.length;
          buffer = Buffer.concat([this._pendingResponseBuffer, buffer], len);
          this._pendingResponseBuffer = Buffer.concat([]);
        }
        if (buffer[0] === 45 || buffer[0] === 43) {
          const firstLineEndIndex = buffer.indexOf(CRLF_BUFFER);
          if (firstLineEndIndex === -1) {
            this._pendingResponseBuffer = Buffer.from(buffer);
            return;
          }
        }
        if (buffer[0] === 45) {
          ...
          new Error(buffer.subarray(5, buffer.indexOf(CRLF_BUFFER)));
          err.eventName = 'error';
          // https://github.com/node-pop3/node-pop3/issues/37
          err.command = this._command.startsWith('PASS ') ? 'PASS ***' : this._command;
          this.emit('error', err);
          return;
        }
        if (buffer[0] === 43) {
          ...
          const [commandName, msgNumber] = (this._command || '').split(' ', 2);
          let stream = null;
          if (MULTI_LINE_COMMAND_NAME.includes(commandName) || !msgNumber && MAYBE_MULTI_LINE_COMMAND_NAME.includes(commandName)) {
            this._updateStream();
            stream = this._stream;
