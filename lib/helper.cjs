...
function normalizeStream2StringOptions(options) {
  ...
  return {
    maxBytes: typeof maxBytes === 'number' && Number.isFinite(maxBytes) && maxBytes > 0 ? maxBytes : undefined,
...
export function stream2String(stream, options) {
  const {
    maxBytes,
    timeoutMs
  } = normalizeStream2StringOptions(options);
...
function dotUnstuff(buffer) {
  ...
  let start = buffer.length >= 2 && buffer[0] === dot && buffer[1] === dot ? 1 : 0;
  let i = start;
  while (i <= buffer.length - 4) {
    if (buffer[i] === cr && buffer[i + 1] === lf && buffer[i + 2] === dot && buffer[i + 3] === dot) {
...
export function listify(str) {
  return str.split(CRLF).filter(Boolean).map(line => line.split(' '));
}
